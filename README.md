...
Artifacts are stored under the `brain` directory, in one UUID-named directory per conversation. Each artifact directly under such a directory has a `<filename>.metadata.json` sidecar (summary, updatedAt, etc); this is how artifacts are discovered, regardless of their own extension. This tool lists and displays those artifacts.
